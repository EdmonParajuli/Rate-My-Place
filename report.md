...
6. **~~`BusinessReviewsPage`'s fix (Bug 1) is a mitigation, not a structural fix.~~ Resolved with genuine server-side cursor pagination.** `BusinessReviewsPage` no longer fetches a fixed batch (`first: 50`) and paginates it client-side — it now fetches `first: 10` at a time and appends via `after`/`fetchMore` on "Load more" (the same `pageInfo`-driven pattern `DiscoverPage` already uses), so the request size never grows with how many reviews a business has, closing off the complexity ceiling entirely rather than just raising it. The numbered page-number UI is gone (a cursor API can't jump to an arbitrary page); replaced with a running "N of Total reviews loaded" line and a "Load more" button. This surfaced a real correctness gap once the "Awaiting Reply" stat and "Needs Response" badge could no longer assume every review was loaded: added a `pendingReplyCount` field to the backend's `businessDashboard` query (`businessDashboardMath.ts`, computed directly from the same reviewCount/repliedCount snapshot already used for `responseRate` — exact, not derived by inverting a rounded percentage), and both UI elements now read that instead of counting through the loaded batch. Also added a distinct empty state for "Needs Response" when the loaded page happens to contain none of the pending ones (they're further back, not actually all replied to) — the old unconditional "All caught up!" would have been misleading there. Verified via `npm run build` (clean typecheck) in both `backend/` and `frontend/`, plus a full `npm run codegen` regen after the schema change.
