...
2. **`ScanAuthModal` has no focus trap.** The overlay blocks mouse interaction correctly, but background elements (Save button, star rating, textarea, Hours toggle) are still `Tab`-reachable for keyboard/screen-reader users, since nothing marks the background `inert` or traps focus. A mouse user can't bypass the gate; a keyboard-only user technically can.

3. **Discover's map view doesn't auto-fit to the listed places.** Defaults to a whole-world Leaflet view; the test places (all in Nepal) aren't visible without manual zoom/pan. Only observed once during this pass — worth a second look before concluding it's the universal default behavior.
...
5. **Native `window.confirm()` for review/draft deletion is inconsistent with the rest of the app's confirmation UX, and is a real testing blind spot.** Every other destructive action (account deletion) uses a custom typed-confirmation component; `MyReviewsPage.tsx`'s delete handlers use a bare `confirm(...)`. It genuinely broke browser automation mid-test — the dialog was undismissable through any available tool and left the tab permanently stuck. The underlying mutation itself was confirmed working via a direct API call instead. Worth reconciling to a custom modal, both for UX consistency and so this flow is actually testable going forward.
